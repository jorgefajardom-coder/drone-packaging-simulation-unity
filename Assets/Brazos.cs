...
public class Brazos : MonoBehaviour
{
    ...
    void EnsureUpToDateFromStreamingAssets(string destPath)
    {
        ...
        if (!shouldCopy)
        {
            try
            {
                RobotPoseContainer existing = JsonUtility.FromJson<RobotPoseContainer>(File.ReadAllText(destPath));
                RobotPoseContainer reference = JsonUtility.FromJson<RobotPoseContainer>(File.ReadAllText(streamingPath));
